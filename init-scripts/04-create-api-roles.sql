...
GRANT SELECT, INSERT, UPDATE, DELETE ON public.libraries TO api_user;
GRANT SELECT ON public.sync_libraries TO api_user;
...
-- Create a function to get current user's library access
-- This will be used for row-level security later
CREATE OR REPLACE FUNCTION public.get_user_library_id()
RETURNS bigint AS $$
BEGIN
    -- Extract library_id from JWT claims
    -- This assumes the JWT contains a library_id claim
    RETURN COALESCE(
...
GRANT EXECUTE ON FUNCTION public.get_user_library_id() TO api_user;
GRANT EXECUTE ON FUNCTION public.get_user_library_id() TO api_anon;
...
COMMENT ON COLUMN public.items.library_id IS 'Library ID this item belongs to';
COMMENT ON COLUMN public.items.library_type IS 'Library type (user or group)';
...
COMMENT ON COLUMN public.collections.library_id IS 'Library ID this collection belongs to';
COMMENT ON COLUMN public.collections.library_type IS 'Library type (user or group)';
...
COMMENT ON COLUMN public.tags.library_id IS 'Library ID this tag belongs to';
COMMENT ON COLUMN public.tags.library_type IS 'Library type (user or group)';
