...
CREATE MATERIALIZED VIEW public.collection_name_hier AS
...
CREATE MATERIALIZED VIEW public.item_type_hier AS
SELECT 
    i.key,
    i.library,
...
CREATE INDEX idx_collection_name_hier_library ON public.collection_name_hier(library);
CREATE INDEX idx_collection_name_hier_parent ON public.collection_name_hier(parent_key);
CREATE INDEX idx_collection_name_hier_level ON public.collection_name_hier(level);

CREATE INDEX idx_item_type_hier_library ON public.item_type_hier(library);
CREATE INDEX idx_item_type_hier_type ON public.item_type_hier(item_type);
CREATE INDEX idx_item_type_hier_parent ON public.item_type_hier(parent_item);
CREATE INDEX idx_item_type_hier_level ON public.item_type_hier(hierarchy_level);

-- Create unique indexes for concurrent refresh capability
CREATE UNIQUE INDEX idx_collection_name_hier_unique ON public.collection_name_hier(key, library);
CREATE UNIQUE INDEX idx_item_type_hier_unique ON public.item_type_hier(key, library);
